...
def read_wallet(json_db, db_env, print_wallet, print_wallet_transactions, transaction_filter):
    ...
    def item_callback(type, d):
        ...
        if type == "name":
            ...
            pubkey = d['public_key']
            json_db['keys'].append({'addr' : addr, 'sec' : sec, 'pubkey': pubkey.encode('hex'), 'privkey': sec})
...
def main():
    ...
    if options.dump is None and options.key is None and options.export is None:
